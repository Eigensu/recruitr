...
def _page_meta(total: int, page: int, limit: int) -> PaginationMeta:
    ...
    return PaginationMeta(page=page, limit=limit, total=total, pages=pages, has_next=page < pages, has_prev=page > 1)
...
async def get_rankings(filters: LeaderboardFilters) -> LeaderboardPage:
    ...
    payload = await fetch_rankings(page=filters.page, limit=filters.limit, search=filters.search, sort_by=filters.sort_by, month=filters.month)
...
async def get_overview() -> LeaderboardOverviewResponse:
    ...
    kpis = [
        LeaderboardKpi(id="total_recruiters", label="Active Recruiters", value=int(summary.get("recruiters", 0)), helper="Competing this month", tone="yellow"),
    ...
    response = LeaderboardOverviewResponse(kpis=kpis, top_recruiter=LeaderboardRecruiterItem.model_validate(top) if top else None)
    await set_cached(key, response.model_dump(mode="json"))
    return response
...
async def get_recruiter(employee_id: str):
    ...
    if payload is None:
        return None
    return {"recruiter": LeaderboardRecruiterItem.model_validate(payload["recruiter"]), "rank_percentile": float(payload["rank_percentile"])}
...
async def get_monthly_growth() -> MonthlyGrowthResponse:
    ...
    response = MonthlyGrowthResponse(labels=payload["labels"], series=[MonthlyGrowthSeries.model_validate(item) for item in payload["series"]])
    await set_cached(key, response.model_dump(mode="json"))
    return response
...
async def get_company_progress() -> CompanyProgressResponse:
    ...
    response = CompanyProgressResponse(items=[CompanyProgressItem.model_validate(row) for row in rows])
    await set_cached(key, response.model_dump(mode="json"))
    return response
...
async def get_activities(page: int, limit: int) -> ActivityPage:
    ...
    return ActivityPage(items=items, meta=_page_meta(payload["total"], payload["page"], payload["limit"]))
...
async def handle_activity(employee_id: str, candidate_id: str | None, activity_type: ActivityTypeEnum, reference_id: str, title: str, description: str) -> bool:
