...
from app.modules.dashboard.models import CandidateMapping, DashboardCandidate, DashboardEmployee, JobOpening
from app.modules.leaderboard.enums import ActivityTypeEnum, BadgeRarityEnum, BadgeTypeEnum
from app.modules.leaderboard.models import Badge, EmployeeStat, LeaderboardHistory, LeaderboardMappings, RecruiterActivity
...
async def seed_leaderboard() -> None:
        ...
        await employees_col.update_one(
            {"email": email},
            {
                "$setOnInsert": {
                    "name": name,
                    "email": email,
                    "created_at": datetime.utcnow() - timedelta(days=120 + idx * 3),
        ...
        await mappings_col.update_one(
            {"candidate_id": candidate["_id"], "job_opening_id": job["_id"]},
            {
                "$set": {
                    ...
                    "mapped_at": datetime.utcnow() - timedelta(days=idx),
                    "updated_at": datetime.utcnow(),
                }
            },
            upsert=True,
    ...
    for idx, (employee, candidate, job, stage) in enumerate(mapping_docs[:18]):
        ...
        await activity_col.update_one(
            {"activity_reference_id": f"seed-{employee['_id']}-{candidate['_id']}-{activity_type.value}"},
            {
                "$set": {
                    ...
                    "activity_reference_id": f"seed-{employee['_id']}-{candidate['_id']}-{activity_type.value}",
                    "created_at": datetime.utcnow() - timedelta(minutes=idx * 18),
                }
            },
            upsert=True,
        )
